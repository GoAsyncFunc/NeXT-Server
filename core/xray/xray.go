...
import (
	...
	vCore "github.com/GoAsyncFunc/NeXT-Server/core"
	"github.com/GoAsyncFunc/NeXT-Server/core/xray/app/dispatcher"
	_ "github.com/GoAsyncFunc/NeXT-Server/core/xray/distro/all"
...
func New(c *conf.XrayConfig, log *conf.LogConfig) (vCore.Core, error) {
	return &Xray{
		Server: getCore(c, log),
		users: &UserMap{
...
func getCore(c *conf.XrayConfig, logConfig *conf.LogConfig) *core.Instance {
	os.Setenv("XRAY_LOCATION_ASSET", c.AssetPath)
	// Log Config
	// AccessLog should be disabled when LogLevel is error or none
	accessLogPath := logConfig.Output
	if logConfig.Level == "error" || logConfig.Level == "none" {
		accessLogPath = ""
	}
	...
	coreLogConfig := &coreConf.LogConfig{
		...
		ErrorLog:    logConfig.Output,
		DNSLog:      logConfig.DnsLog,
		MaskAddress: logConfig.MaskAddress,
	}
	// DNS config
